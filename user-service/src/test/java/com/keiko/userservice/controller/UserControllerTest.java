...
class UserControllerTest {
    ...
    void addUserRoles_should_successfully () throws Exception {
        UserRolesRequest request = new UserRolesRequest (1L, Set.of (1L));
        mockMvc.perform (post (USER_BASE + ADD_ROLES)
                .content (objectMapper.writeValueAsString (request))
                .contentType (APPLICATION_JSON_VALUE))
                .andExpect (status ().isOk ());

        verify (userService, times (1)).addRoles (any (UserRolesRequest.class));
        verifyNoMoreInteractions (userService);
    }
    ...
    void deleteUserRoles_should_successfully () throws Exception {
        ...
        verify (userService, times (1)).deleteRoles (any (UserRolesRequest.class));
        verifyNoMoreInteractions (userService);
    }
}
